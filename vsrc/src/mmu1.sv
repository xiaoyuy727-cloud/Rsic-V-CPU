...
`ifdef VERILATOR
    if (cpu_req_eff.valid && cpu_resp.data_ok) begin
        $display("[MMU_BUG_REACCEPT_ON_RESP] cpu_addr=%h cpu_resp_data=%h cur=%0d saved_addr=%h",
                 cpu_req_eff.addr, cpu_resp.data, cur, saved_req.addr);
    end
`endif
